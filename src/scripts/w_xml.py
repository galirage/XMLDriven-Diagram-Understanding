...
def fetch_diagram_data(state: State):
    ...
    loader = ExcelAutoshapeLoader("data/diagram.xlsx")
    loader.load()
    diagram_json = loader.export()
    with open("data/diagram_json.json", "w") as f:
...
def run_w_xml():
    ...
    user_question = "What is the red connector connected to?"
    state = State(
        autoshape_data="",
        shape_data="",
        connector_data="",
